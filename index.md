---
layout: home
title: ByeByeBryan
lede: Building games, graphics experiments, and AI tooling.
eyebrow: dev.byebyebryan.com
sections:
  - title: Projects
    projects:
      - name: gomoku2d
        description: An old favorite, built properly. A retro Gomoku game built on a modern web and Rust stack.
        url: https://github.com/byebyebryan/gomoku2d
        image: /assets/projects/gomoku2d.gif
        image_alt: Gomoku2D local match in progress on a pixel-art board.
        links:
          - label: play
            url: https://dev.byebyebryan.com/gomoku2d/
      - name: lazy-serializable
        description: Header-only C++ serialization — declare fields once, get JSON, Binary, YAML, and more for free.
        url: https://github.com/byebyebryan/lazy-serializable
  - title: Notes
    body: >
      Short entries on things worth writing down.
---
